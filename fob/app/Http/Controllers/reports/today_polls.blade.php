...
<table width="100%" border="1" cellpadding="10px" class="col-md-offset-2 col-md-10">
 ...
 <tr><th colspan="7" style="text-align: center">Today Polls</th></tr>
...
@foreach($today_polls as $today_poll)
    <tr style="height:20px">
        <td>{{ $today_poll['id'] }}</td>
        <td>{{ $today_poll['question'] }}</td>
        <td>{{ $today_poll['from_date'] }}</td>
        <td>{{ $today_poll['to_date'] }}</td>
        <td>{{ $today_poll['active'] }}</td>
    </tr>
@endforeach
</table>
