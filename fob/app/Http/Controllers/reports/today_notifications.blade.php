...
@foreach($today_notifications as $today_notification)
    <tr style="height:20px">
        <td>{{ $today_notification['id'] }}</td>
        <td>{{ $today_notification['description'] }}</td>
        <td>{{ $today_notification['from_date'] }}</td>
        <td>{{ $today_notification['to_date'] }}</td>
        <td>{{ $today_notification['active'] }}</td>  
    </tr>
@endforeach
</table>
