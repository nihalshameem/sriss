...
<tr><th colspan="7" style="text-align:left;font-size: 18px;"><a href="{{ URL::previous() }}" style="background-color: red;color:white;margin-left: 20px;padding:10px">Back</a></th></tr>
     <tr><th colspan="7" style="text-align: center">Today Members</th></tr>
...
@foreach($today_members as $today_member)
    <tr style="height:20px">
        <td>{{ $today_member['member_id'] }}</td>
        <td>{{ $today_member['name'] }}</td>
        <td>{{ $today_member['mobile_number'] }}</td>
        <td>{{ $today_member['dob'] }}</td>
        <td>{{ $today_member['sex'] }}</td>
        <td>{{ $today_member['email'] }}</td>
        <td>{{ $today_member['address'] }}</td>   
    </tr>
@endforeach
</table>
