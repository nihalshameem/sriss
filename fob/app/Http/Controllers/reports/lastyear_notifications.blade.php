...
<table width="100%" border="1" cellpadding="10px" class="col-md-offset-2 col-md-10">
 ...
 <tr><th colspan="7" style="text-align: center">Past year Notifications</th></tr>
...
@foreach($lastyear_notifications as $lastyear_notification)
    <tr style="height:20px">
        <td>{{ $lastyear_notification['id'] }}</td>
        <td>{{ $lastyear_notification['description'] }}</td>
        <td>{{ $lastyear_notification['from_date'] }}</td>
        <td>{{ $lastyear_notification['to_date'] }}</td>
        <td>{{ $lastyear_notification['active'] }}</td>  
    </tr>
@endforeach
</table>
