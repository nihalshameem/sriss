...
<table width="100%" border="1" cellpadding="10px" class="col-md-offset-2 col-md-10">
    <tr><th colspan="7" style="text-align:left;font-size: 18px;"><a href="{{ URL::previous() }}" style="background-color: red;color:white;margin-left: 20px;padding:10px">Back</a></th></tr>
     <tr><th colspan="7" style="text-align: center">Past Month Members</th></tr>
...
@foreach($lastmonth_members as $lastmonth_member)
    <tr style="height:20px">
        <td>{{ $lastmonth_member['member_id'] }}</td>
        <td>{{ $lastmonth_member['name'] }}</td>
        <td>{{ $lastmonth_member['mobile_number'] }}</td>
        <td>{{ $lastmonth_member['dob'] }}</td>
        <td>{{ $lastmonth_member['sex'] }}</td>
        <td>{{ $lastmonth_member['email'] }}</td>
        <td>{{ $lastmonth_member['address'] }}</td>   
    </tr>
@endforeach
</table>
