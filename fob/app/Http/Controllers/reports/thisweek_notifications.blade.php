...
<table width="100%" border="1" cellpadding="10px" class="col-md-offset-2 col-md-10">
 ...
 <tr><th colspan="7" style="text-align: center">Past week Notifications</th></tr>
...
@foreach($thisweek_notifications as $thisweek_notification)
    <tr style="height:20px">
        <td>{{ $thisweek_notification['id'] }}</td>
        <td>{{ $thisweek_notification['description'] }}</td>
        <td>{{ $thisweek_notification['from_date'] }}</td>
        <td>{{ $thisweek_notification['to_date'] }}</td>
        <td>{{ $thisweek_notification['active'] }}</td>  
    </tr>
@endforeach
</table>
