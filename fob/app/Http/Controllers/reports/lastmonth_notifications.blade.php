...
<table width="100%" border="1" cellpadding="10px" class="col-md-offset-2 col-md-10">
 ...
 <tr><th colspan="7" style="text-align: center">Past Month Notifications</th></tr>
...
@foreach($lastmonth_notifications as $lastmonth_notification)
    <tr style="height:20px">
        <td>{{ $lastmonth_notification['id'] }}</td>
        <td>{{ $lastmonth_notification['description'] }}</td>
        <td>{{ $lastmonth_notification['from_date'] }}</td>
        <td>{{ $lastmonth_notification['to_date'] }}</td>
        <td>{{ $lastmonth_notification['active'] }}</td>  
    </tr>
@endforeach
</table>
