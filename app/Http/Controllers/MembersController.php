...
class MembersController extends ApiController
{
    ...
    {
        $subscription_user_count = Compliance::where('Compliance_id', '11')->first();
        ...
        } elseif ($member >= $subscription_user_count->Compliance_text) {
            return Response([
                'status' => 'failure',
                'code' => 400,
                'message' => 'Subscription User Count Exceeded. Please Contact the Admin',
            ]);
        } else {
                ...
                if ($memberactive->Member_Active_Flag == 'Y') {
                    if ($memberactive->Member_Approved_Flag == 'Y') {
                        ...
                        return Response([
                            'status' => 'success',
                            'code' => 200,
                            'message' => 'தங்களது உறுப்பினர் ஒப்புதல் செயலில் உள்ளது.24 மணி நேரம் கழித்து உள் நுழைய முயற்சி செய்யவும், பதிவு செய்து 24 நான்கு மணி  நேரத்திற்கு பிறகும் உள் நுழைய முடியவில்லையனில் இந்த எண்ணை  +91 98405 15510 தொடர்பு கொள்ளவும்.Your approval is in progress, please access the App after 24 hours, if you are not able to login after 24 hours please Contact  +91 98405 15510',
                            'IsApproval' => false,
                        ]);
                    }
                } else {
                    return Response([
                        'status' => 'failure',
                        'code' => 400,
                        'message' => 'Your Membership Deactivated, Please contact Admin',
                        'IsApproval' => false,
                    ]);
                }
    ...
    public function MemberEdit($id)
    {
        ...
        $group_ids = MemberGroup::where('active', 'Y')->pluck('Group_id');
        $member = Member::where('Member_Id', $id)->first();
        $member->groups = GroupMembers::whereIn('Group_id', $group_ids)
            ->where('Member_tbl_id', $member->id)->where('active', 'Y')
            ->get();
        $category = MemberCategory::where('MemberCategory_id', $member->Member_Category_Id)->first();
        return view('Member.member_edit', compact('memberGroups', 'member', 'category'));
    }
    ...
    public function membersearch(Request $request)
    {
        if ($request->ajax()) {
            ...
            } else if ($request->memberCategory == null && $request->membersearch == null) {
                $members = Member::get();
            } else {
