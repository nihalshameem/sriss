...
<table width="100%" cellpadding="10px" class="col-md-offset-2 col-md-10">
    <tr style="border-bottom:none"><th colspan="8" style="text-align: center;padding:10px;font-size:20px">Past year Polls<span class="label label-success">{{ count($lastyear_polls) }}</span></th></tr>
    <tr style="padding:50px;height:50px">        
        ...
        <a href="/lastyear_polls_xlsx" style="color:purple;margin-left: 20px;padding:10px;font-size:25px"><i class="fa fa-download" aria-hidden="true"></i></a></td>
        </center>
    </tr>
...
@foreach($lastyear_polls as $lastyear_poll)
    <tr style="height:20px;border:1px solid grey">
        ...
        <td style="padding:10px;text-align:center;border:1px solid grey">{{ $lastyear_poll['created_at'] }}</td>
        
        
        
        <td style="padding:10px;text-align:center;border:1px solid grey">{{ substr($lastyear_poll['question'],0,100) }}...<br>
        
        @foreach($lastyear_poll['response'] as $key => $response )
            <ul>
                <li>{{ $lastyear_poll['response'][$key] }} = {{ $lastyear_poll['response_count'][$key] }} </li>
            </ul>
        @endforeach
        ...
        <td style="padding:10px;text-align:center;border:1px solid grey">{{ $lastyear_poll['from_date'] }}</td>
        <td style="padding:10px;text-align:center;border:1px solid grey">{{ $lastyear_poll['to_date'] }}</td>
        <td style="padding:10px;text-align:center;border:1px solid grey">{{ $lastyear_poll['active'] }}</td>
    </tr>
@endforeach
</table>
