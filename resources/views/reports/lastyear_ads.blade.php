...
<table width="100%" cellpadding="10px" class="col-md-offset-2 col-md-10">
    <tr style="border-bottom:none"><th colspan="8" style="text-align: center;padding:10px;font-size:20px">Past year Advertisements<span class="label label-success">{{ count($lastyear_ads) }}</span></th></tr>
    <tr style="padding:50px;height:50px">        
        ...
        <a href="/lastyear_ads_xlsx" style="color:purple;margin-left: 20px;padding:10px;font-size:25px"><i class="fa fa-download" aria-hidden="true"></i></a></td>
        </center>
    </tr>
...
@foreach($lastyear_ads as $lastyear_ad)
    <tr style="height:20px;border:1px solid grey">
        ...
        <td style="padding:10px;text-align:center;border:1px solid grey">{{ $lastyear_ad['created_at'] }}</td>
        <td style="padding:10px;text-align:center;border:1px solid grey">{{ substr($lastyear_ad['description'],0,100) }}...</td>
        <td style="padding:10px;text-align:center;border:1px solid grey">{{ $lastyear_ad['company'] }}</td>
        <td style="padding:10px;text-align:center;border:1px solid grey">{{ $lastyear_ad['link'] }}</td>
        <td style="padding:10px;text-align:center;border:1px solid grey">{{ $lastyear_ad['from_date'] }}</td>
        <td style="padding:10px;text-align:center;border:1px solid grey">{{ $lastyear_ad['to_date'] }}</td>
        <td style="padding:10px;text-align:center;border:1px solid grey">{{ $lastyear_ad['active'] }}</td>
    </tr>
@endforeach
