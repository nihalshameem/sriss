...
<table width="100%" cellpadding="10px" class="col-md-offset-2 col-md-10">
    ...
    <tr style="padding:50px;height:50px">        
        ...
        <a href="/yesterday_polls_xlsx" style="color:purple;margin-left: 20px;padding:10px;font-size:25px"><i class="fa fa-download" aria-hidden="true"></i></a></td>
        </center>
    </tr>
...
@foreach($yesterday_polls as $yesterday_poll)
    <tr style="height:20px;border:1px solid grey">
        ...
        <td style="padding:10px;text-align:center;border:1px solid grey">{{ $yesterday_poll['id'] }}</td>
        
        
        <td style="padding:10px;text-align:center;border:1px solid grey">{{ substr($yesterday_poll['question'],0,100) }}...<br>
        
        @foreach($yesterday_poll['response'] as $key => $response )
            <ul>
                <li>{{ $yesterday_poll['response'][$key] }} = {{ $yesterday_poll['response_count'][$key] }} </li>
            </ul>
        @endforeach
