...
<table width="100%" cellpadding="10px" class="col-md-offset-2 col-md-10">
    <tr style="border-bottom:none"><th colspan="8" style="text-align: center;padding:10px;font-size:20px">This year Notifications<span class="label label-success">{{ count($thisyear_notifications) }}</span></th></tr>
    <tr style="padding:50px;height:50px">        
        ...
        <a href="/fob/thisyear_notifications_xlsx" style="color:purple;margin-left: 20px;padding:10px;font-size:25px"><i class="fa fa-download" aria-hidden="true"></i></a></td>
        </center>
    </tr>
...
@foreach($thisyear_notifications as $thisyear_notification)
    <tr style="height:20px;border:1px solid grey">

        <td style="padding:10px;text-align:center;border:1px solid grey">{{ $i++ }}</td>
       <td style="padding:10px;text-align:center;border:1px solid grey">{{ $thisyear_notification['created_at'] }}</td>
        <td style="padding:10px;text-align:center;border:1px solid grey">{{ $thisyear_notification['description'] }}</td>
       <td style="padding:10px;text-align:center;border:1px solid grey">{{ $thisyear_notification['from_date'] }}</td>
        <td style="padding:10px;text-align:center;border:1px solid grey">{{ $thisyear_notification['to_date'] }}</td>
        <td style="padding:10px;text-align:center;border:1px solid grey">{{ $thisyear_notification['active'] }}</td>  
    </tr>
@endforeach
</table>
